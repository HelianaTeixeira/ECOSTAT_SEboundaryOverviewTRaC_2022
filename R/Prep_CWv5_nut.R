...
# V5cw: adapted to Coastal waters data (by HT): names of var and IC common types & Marine regions
...
Temp1 <- dat.u %>%     # use dat.u.d for number of distinct standards
  filter(!is.na(ValueUpStd) & RangeRep==TRUE,!is.na(ICcode.TRAC)) %>%
  droplevels() %>% 
  group_by(Country) %>% 
  count() %>%
  droplevels() %>% 
...
OverV1 <- select(dat.CW,ICcode.TRAC,ICType) %>% filter(!is.na(ICcode.TRAC)) %>% group_by() %>% distinct() 
...
# account for salinity gradient (already in long format)
dat.shrt <- dat.u.m%>%
  select(All.obs,GIG,MarineRegion,ICcode.TRAC,Country,NatType,physChemText,SumMetric,ValueStd,ValueUpStd,UnitUsed,RangeRep,ReasonRange,DepthZone,MetricType,Sal,Salinity)%>% 
  droplevels()
dim(dat.shrt)
