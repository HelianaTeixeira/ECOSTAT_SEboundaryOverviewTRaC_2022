...
<!-- leveneTest(Value ~ ICcode.TRAC, data=datL.u.d,subset = n>2 )# dropped "ICcode.TRAC" -->
<!-- leveneTest(log(Value) ~ ICcode.TRAC, data=datL.u.d,subset = n>2 )# dropped "ICcode.TRAC" -->


<!-- # create factorial model using only IC types with more than 2 countries in the group (dropped "ICcode.TRAC") -->
<!-- mod.L <- aov(log(Value) ~ ICcode.TRAC, data=datL.u.d,subset =  Range =="L") -->
<!-- mod.U <- aov(log(Value) ~ ICcode.TRAC, data=datL.u.d,subset = Range =="U") -->
<!-- # look at residuals -->
...
<!-- Table.L <- my_anova.L %>% mutate(term = c("Intercept","GIG","Residuals")) # dropped "ICcode.TRAC" -->
...
<!-- # Table.U <- my_anova.U %>% mutate(term = c("Intercept","GIG","Residuals"))# dropped "ICcode.TRAC" -->
...
Very few values are available to allow meaningful statistical comparison accross grouping factor(s), so no analysis of variance was applied to test the significance of GIG, IC Type, Country or other grouping factor effect. 
