...
```{r}
load(here("Data","CoastalSecchi disk depth.RData"))
...
#labels for legends
fig_ref1 <- "Figure 3.9"
fig_ref2 <- "Figure 3.10"
fig_ref3 <- "Figure 3.11"
tab_ref1 <- "Table 3.?"
tab_ref2 <- "Table 3.?"
...
FCap3 <- paste(fig_ref3,"Coastal",physChemText.u,"standards' comparing the range of ",physChemText.u," in the data when oultier (PT standard at 35m, 95th percentile) is removed.")
#or
FCap3 <- "Coastal Secchi disk depth standards’ comparing the range of Secchi disk depth in the data when outlier (PT standard at 35m, 95th percentile) (top) is removed (bottom)."
TCap1 <- paste(tab_ref1,"Analysis of variance for factorial model relating country to member state boundary values for lower groups of log ",physChemText.u, " boundaries (Including main and partial effect sizes, Omega squared.")#and upper
...
In general, few values are available to allow meaningful statistical comparison across grouping factor(s), so no analysis of variance was applied to test the significance of GIG, IC Type, Country or other grouping factor effect. 
...
```{r, fig.width = 7, fig.height =8.3, fig.cap=FCap3, warning = FALSE}
