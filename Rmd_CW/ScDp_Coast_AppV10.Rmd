...
dat.u.m$ValueUpStd <- as.numeric(dat.u.m$ValueUpStd)
dat.u <- dat.u.m
```
...
P2 <- ggplot(datL.u.d, aes(x=ICcode.TRAC, y=Value,color=Range,shape=physChemStandard2.u))+
  geom_point(data = subset(datL.u.d, !is.na(ReasonRange)))+
  scale_shape_manual(values = symb, name ="")+
  scale_color_manual(values = c("blue","red"))+
      ylab(ylb)+xlab("")+
    theme_bw() +
  geom_point(data = subset(datL.u.d,RangeRep==FALSE), mapping = 
                 aes(x = ICcode.TRAC, y = Value, shape = physChemStandard2.u),color="black",position = position_nudge(x= -0.1 ,y = 0))+
    theme(legend.position = "bottom",
          legend.text = element_text(color = "black"))+
...
```{r PT extreme value, warning=FALSE}
#Dot plots of "ValueStd" & "ValueUpStd" by "Country" & "GIG"
#plot also with PT extreme Ref Cond outlier
which(grepl(11166, datL.u.d$All.obs)) #find PT All.obs=="11166" row number to duplicate: 64

##duplicate PT entry previously removed
datL.u.d.outlier <- datL.u.d%>% 
  filter(row_number()==64)%>%
  bind_rows(datL.u.d)

which(grepl(11166, datL.u.d.outlier$All.obs)) #duplicated PT entries row numbers: 1, 65
#plot data with & without PT outlier
datL.u.d.outlier <- mutate(datL.u.d.outlier, 
                           col = case_when(row_number()==1 ~ "ValueUpStd", TRUE ~ col),
                           Value = case_when(row_number()==1 ~ 35.00, TRUE ~ Value),
                           Range = case_when(row_number()==1 ~ "U", TRUE ~ Range))

outl.in <- ggplot(subset(datL.u.d.outlier,RangeRep==TRUE), aes(x=Country, y=Value,color=Range,shape=physChemStandard2.u))+
  geom_point(position = position_nudge(x= +0.1 ,y = 0))+
  scale_shape_manual(values = symb, name ="")+
  scale_color_manual(values = c("blue","red"))+
      ylab(ylb)+xlab("")+
    theme_bw() +
    geom_point(data = subset(datL.u.d.outlier,RangeRep==FALSE), mapping = 
                 aes(x = Country, y = Value, shape = physChemStandard2.u),color="black",position = position_nudge(x= -0.1 ,y = 0))+
    theme(legend.position = "bottom", legend.box="vertical", legend.margin=margin(),
          legend.text = element_text(color = "black")) +
...
save(dat.u.d,Metrics,Temp,Temp1,Temp2,OverV4,RangeCntry,ValCntry,TypeAllCntry,NinGroup,CntryInGroup,GrpGT2,symb,ylb,dec,physChemText.u,dat.u.d,dat.u,datL.u.d,qA25,qA50,qA75,qA90,min.x,max.x,qA10U,qA25U,qA50U,qA75U,qA90U,qA10L,qA25L,qA50L,qA75L,qA90L,GrpGT2.Q,P1,P2,outl.in,outl.out,coP1,coP2,coP3,coP4,file=OutFile)
