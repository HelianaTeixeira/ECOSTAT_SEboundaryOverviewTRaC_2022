...
physChemText.u <- "TN"
ylb <- "TN (mg/L) GM boundary"
#CV <- "" # critical value to highlight:
...
#ValueUpStd == 6 is an eRROR drop 2 entries from EE: All.obs==6550 & All.obs==6569
```
...
dat.u.m <- dat.u %>%
  mutate(ValueUpStd = case_when(
    All.obs==6550 ~ NA_real_,
    All.obs==6569 ~ NA_real_,
    TRUE ~ ValueUpStd))
  #check
dat.u.m%>%
  filter(!is.na(ValueUpStd), Country=="EE")%>%
  count(Country, ValueUpStd)

#RangeRep is TRUE for  2 DE entries:
dat.u.m<- dat.u.m%>%
  mutate(RangeRep = case_when(
   Country =="DE" & !is.na(ValueUpStd) ~ TRUE, 
    TRUE ~ RangeRep))
...
dat.u.m%>%
  ...
  count(Country, ValueUpStd)
    ...
    #assign sal values to 6 SE entries non-spread
    dat.u.m<-dat.u.m%>%
      mutate(ValueUpStd=case_when(
...
#2nd transpose salinity and correspondent value ranges not to be assumed as a L-U range
dat.u.m <- dat.u.m%>%
  mutate(RangeRep = case_when(
...
# edit fields
dat.u.m <- dat.u.m %>%
  mutate(Sal =ifelse(is.na(Salinity),NA, Sal))# 104 with Salinity vals

#preserve entries of SE (6 records) with salinity not in range: change Sal level
dat.u.m <- dat.u.m%>%
    mutate(Sal = case_when(Country == "SE" & Sal=="Salinity2" & is.na (ValueUpStd) ~ "Salinity1", TRUE ~ Sal))

dat.u.m <- dat.u.m%>%
  mutate(ValueStd = case_when(Sal=="Salinity2" ~ ValueUpStd, TRUE ~ ValueStd))

dat.u.m <- dat.u.m%>%
  mutate(ValueUpStd = case_when(Sal=="Salinity1" ~ NA_real_ , TRUE ~ ValueUpStd))

dat.u.m <- dat.u.m %>%
  mutate(ValueUpStd = case_when(Country=="SE" ~ NA_real_, TRUE ~ ValueUpStd))

dat.u.m  <- dat.u.m %>%
  filter(!((All.obs == 11440 & is.na(Salinity) & is.na(Sal)) |
             (All.obs == 11441 & is.na(Salinity) & is.na(Sal)) |
             (All.obs == 11442 & is.na(Salinity) & is.na(Sal)) |
             (All.obs == 11006 & is.na(Salinity) & is.na(Sal))
           ))

#eliminate duplicated rows in dataframe
dat.u.m<- distinct(dat.u.m)
...
#Edit the next line
symb <- c(19,7,10) #try symb1 if code failing
...
## 3.3.3.1 Total N (coastal waters) 
...
## A5.1 Appendix Total Nitrogen (coastal waters)
...
```{r fig.width = 5, fig.height =7, fig.cap=CapA3}
#scatterplot all vals
salP1<- ggplot(dat.u,aes(x=Salinity,y=ValueStd))+
  geom_point()+theme_bw()+
  ylab("TN (mg/L) GM boundary")
```
