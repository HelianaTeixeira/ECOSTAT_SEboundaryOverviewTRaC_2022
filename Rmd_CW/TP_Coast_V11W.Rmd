...
#CV <- "" # critical value to highlight: 800(µg/L)
...
#Select standard
dat.u <- dat.CW %>% 
  filter(physChemText==physChemText.u,!is.na(ValueStd),UnitUsed!="mg/Kg (sed.)") %>% 
...
dat.u.m <- dat.u.m%>%
   mutate(ValueUpStd = case_when(All.obs==8649 ~ NA_real_ , TRUE ~ ValueUpStd)) #keep all other ValueUpStd as were
...
dim(dat.u.m) #206 entries
#elimindate duplicated rows in dataframe
dat.u.m<- distinct(dat.u.m) #157 entries
...
salP1<- ggplot(dat.u,aes(x=Salinity,y=ValueStd))+
  ...
  scale_colour_discrete(limits = c("NO","LT","SE"),name="Country")+#limits = c("NO","LT","SE"),
  theme_bw()+
  theme(legend.position = "bottom")+
...
## A7.1 Appendix Total phosphorus (coastal waters)
