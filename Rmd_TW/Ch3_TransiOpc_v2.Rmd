...
```{r}
#load data needed
load(here("Data","Transitional% oxygen saturation.RData"))
...
#Legends for report
FCap1 <- paste(fig_ref1,"Comparison of transitional",physChemText.u,"standards by country. (single value black, minimum blue, maximum red symbols, dotted lines show interquartile range of mean or median values, (10th quantile=red, 25th quantile=orange, 50th quantile = blue.)")
FCap2 <- paste(fig_ref2,"Transitional",physChemText.u,"standards  by IC type (single value black, minimum blue, maximum red. Horizontal lines mark the 25th (green) and 75th (red) quantiles, (excluding standards based on quantiles).")
...
TCap2 <- paste(tab_ref2,"Overview of common types showing the number of MS/national types/distinct standards for transitional waters", physChemText.u,".")
...
# 
# mod <- aov(log(Value) ~ Country, data=SumDat.l) # create factorial model using only IC types with more than 2 countries in the group
# 
...
Very few values are available to allow meaningful statistical comparison across grouping factor(s), so no analysis of variance was applied to test the significance of GIG, IC Type, Country or other grouping factor effect.
