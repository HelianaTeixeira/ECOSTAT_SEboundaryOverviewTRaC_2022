...
#CV <- "" # critical value to highlight: 165(µg/L), 500(µg/L)
...
dat.u.m <- dat.u.m%>%
   mutate(ValueUpStd = case_when(All.obs==8644 ~ NA_real_ , TRUE ~ ValueUpStd)) #keep all other ValueUpStd as were
...
dim(dat.u.m) #132 entries
#elimindate duplicated rows in dataframe
dat.u.m<- distinct(dat.u.m) #109 entries
...
## 3.6.2 Orthophosphate (transitional waters) 
...
#  geom_segment(data=GrpGT2.Q,mapping=aes(x=seq(0.75,dim(GrpGT2.Q)[1]-0.25,1.0),y=q25u,xend=seq(1.25,dim(GrpGT2.Q)[1]+0.25,1.0),yend=q25u),inherit.aes = FALSE,colour = "green",size = 1.5)+
#  geom_segment(data=GrpGT2.Q,mapping=aes(x=seq(0.75,dim(GrpGT2.Q)[1]-0.25,1.0),y=q75u,xend=seq(1.25,dim(GrpGT2.Q)[1]+0.25,1.0),yend=q75u),inherit.aes = FALSE,colour = "red",size = 1.5)+
#   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+ ylim(NA, 40)                             
```
...
salP1<- ggplot(dat.u,aes(x=Salinity,y=ValueStd))+
  ...
  scale_colour_discrete(limits = c("BE","IE","SE","PT"), name="Country")+#limits = c("SE", "NO","PT")
  theme_bw()+
  theme(legend.position = "bottom")+
...
## A6.2 Appendix Orthophosphate (transitional waters)



```{r fig.width = 8.23, fig.height =5, fig.cap=CapA1}
bk <- c(0.1,10,50,200)
ymin <-0.1
ymax <- 200
