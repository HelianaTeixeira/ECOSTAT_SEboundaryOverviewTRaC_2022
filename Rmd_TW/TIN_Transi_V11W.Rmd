...
CV <- ">2" # critical value to highlight: (HR and UK; 10 fold higher)
...
Cap1 <- paste(LCap1,"Comparison of transitional",physChemText.u,"standards by country (single value black, minimum blue, maximum red symbols).")
Cap2 <- paste(LCap2,"transitional",physChemText.u,"standards  by IC type (single value black, minimum blue, maximum red). Horizontal lines mark 25th (green) and 75th (red) quantiles, (excluding standards based on quantiles).")
...
CapT2 <- paste(LCapT2,"Overview of common types showing the number of country/national types/distinct standards for transitional", physChemText.u)
...
dat.TW <- read.csv(file = FName.dat, header = TRUE, fileEncoding = "latin1")
...
dat.u.m <- dat.u %>%
  ...
  #check
dat.u.m%>%
  filter(ReasonRange == "salinity range", Country=="SE")%>%
  count(Country, ValueUpStd)

# duplicate those 2 SE entry records and edit fields
dat.u.m <- rbind(dat.u.m,dat.u.m[rep(rownames(dat.u.m[dat.u.m$All.obs==11799,]),1),])
dat.u.m[which(dat.u.m$All.obs==11799)[1],"ValueUpStd"] <- NA
dat.u.m[which(dat.u.m$All.obs==11799)[1],"Salinity2"] <- NA
dat.u.m[which(dat.u.m$All.obs==11799)[2],"ValueStd"]  <- 0.34036281
dat.u.m[which(dat.u.m$All.obs==11799)[2],"Salinity1"] <- 0
dat.u.m[which(dat.u.m$All.obs==11799)[2],"ValueUpStd"] <- NA

dat.u.m <- rbind(dat.u.m,dat.u.m[rep(rownames(dat.u.m[dat.u.m$All.obs==11793,]),1),])
dat.u.m[which(dat.u.m$All.obs==11793)[1],"ValueUpStd"] <- NA
dat.u.m[which(dat.u.m$All.obs==11793)[1],"Salinity2"] <- NA
dat.u.m[which(dat.u.m$All.obs==11793)[2],"ValueStd"]  <- 0.26332596
dat.u.m[which(dat.u.m$All.obs==11793)[2],"Salinity1"] <- 0
dat.u.m[which(dat.u.m$All.obs==11793)[2],"ValueUpStd"] <- NA
...
symb <- c(4,8,19,17,3,10) #try symb1 if code failing
...
<!-- #code for two final graphs in Annex -->
...
## 3.3.2.2 Total Inorganic Nitrogen (transitional waters) 
...
DP2()+
 ...
 geom_segment(data=GrpGT2.Q,mapping=aes(x=seq(0.75,dim(GrpGT2.Q)[1]-0.25,1.0),y=q75u,xend=seq(1.25,dim(GrpGT2.Q)[1]+0.25,1.0),yend=q75u),inherit.aes = FALSE,colour = "red",size = 1.5)+
  scale_x_discrete(labels = c("TW-BT1" = "TW\nBT1",
                              "TW-NEA11"="TW\nNEA11",
                              "TW-Estuaries"="TW\nEstuaries",
                              "TW-CoastalLagoonsPolyeuhaline"="TW\nCoastal Lagoons\nPoly-euhaline",
                              "inapplicable"="inapplicable"))+
  theme(axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=0.5))#change legend as needed
```
...
## A3.2 Appendix Total Inorganic Nitrogen (transitional waters)
...
# add lower point of range as blue symbol  
    ...
    ylab(ylb)+xlab("")+
    facet_wrap(~ ICcode.TRAC,ncol=3,nrow=8, scales="free_x") +
    theme_bw() +
