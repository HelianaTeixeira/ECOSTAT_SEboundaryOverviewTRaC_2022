...
CV <- "7" # critical value to highlight: ES: 7 (mg/l), 10 (mg/l)
...
#ValueStd == 10 mg/L (>= 97.5%; 2 entries) from ES, but maybe also 7mg/L (>= 92.5%) from ES
```
...
#2nd (country specific - edit) salinity range and vaueUpstd but no salinity 2 : LT 3 entries
dat.u.m <- dat.u.m%>%
...
dat.u.m<- distinct(dat.u.m) #44 entries
...
plot1_ylim<-plot1+ ylim(NA, 2.6) #adjust as need 
...
#Edit the next line
symb <- c(4,19,14,7,10) #try symb1 if code failing
...
## 3.5.2 Total N (transitional waters) 
...
DP2()+
 ...
 geom_segment(data=GrpGT2.Q,mapping=aes(x=seq(0.75,dim(GrpGT2.Q)[1]-0.25,1.0),y=q75u,xend=seq(1.25,dim(GrpGT2.Q)[1]+0.25,1.0),yend=q75u),inherit.aes = FALSE,colour = "red",size = 1.5)+
  ...
  scale_x_discrete(labels = c("TW-BT1" = "TW\nBT1",
                              ...
                              "inapplicable"="inapplicable"))#change legend as needed
```
...
## A5.2 Appendix Total Nitrogen (transitional waters)



```{r fig.width = 7, fig.height =5, fig.cap=CapA1}
bk <- c(0.1,0.3,1,3,10)
ymin <-0.1
ymax <- 10
...
salP1<- ggplot(dat.u,aes(x=Salinity,y=ValueStd))+
  ...
  scale_colour_discrete(limits = c("BE","LT","NL","SE"),name="Country")+#limits = c("BE","LT","SE"),
  theme_bw()+
  theme(legend.position = "bottom")+
   ylab("TN (mg/L) GM boundary")#ylim(NA, 70) 
salP1
