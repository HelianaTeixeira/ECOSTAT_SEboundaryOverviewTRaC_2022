...
physChemText.u <- "TP"
ylb <- "TP (µg/L) GM boundary"
#CV <- "1000" # critical value to highlight: 1000 & 2000 (ES)
...
CapA3 <- paste(LCapA3,physChemText.u, "standards for which a salinity gradient was associated.") 
...
#ValueStd == 1000, 2000 ug/L from ES
```
...
dat.u.outdrop<-dat.u%>%filter(ValueStd<200)
df_out<-dat.u.outdrop
BPChk(df_out)
```
...
dat.u.m <- dat.u
#(country specific) drop vals >200
dat.u.m<-dat.u.m%>%filter(ValueStd<200)
#(country specific) correct entry 
dat.u.m <- dat.u.m%>%
   mutate(ValueUpStd = case_when(All.obs==8648 ~ NA_real_ , TRUE ~ ValueUpStd)) #keep all other ValueUpStd as were
...
dat.u.m%>%
  ...
  count(Country, ValueUpStd)
    ...
    # (country specific - edit)assign sal values to entries non-spread: 1 BE
    dat.u.m<-dat.u.m%>%
    ...
    # check
    dat.u.m%>%
      filter(ReasonRange == "salinity range", !is.na(Salinity2))%>%
      count(Country, ValueUpStd)
...
dim(dat.u.m) #64 entries
#elimindate duplicated rows in dataframe
dat.u.m<- distinct(dat.u.m) #43 entries
...
source(here("R","Prep_TWv5_nut.R"))  # runs chunk of code to prepare the data
...
# mod2 <- aov(Value ~ Country, data=SumDat.l,subset = n>2) # create factorial model
# # for one-way between subjects designs, partial omega squared is equvilant to omega squared
# # look at residuals
...
# plot1_ylim<-plot1+
#   geom_hline(yintercept = c(q50All,q75All,q90All),linetype="dashed", color=c("blue","orange","red"))+ ylim(NA,50)
# 
# gridExtra::grid.arrange(plot1, plot1_ylim)
```
...
#  geom_segment(data=GrpGT2.Q,mapping=aes(x=seq(0.75,dim(GrpGT2.Q)[1]-0.25,1.0),y=q25u,xend=seq(1.25,dim(GrpGT2.Q)[1]+0.25,1.0),yend=q25u),inherit.aes = FALSE,colour = "green",size = 1.5)+
#  geom_segment(data=GrpGT2.Q,mapping=aes(x=seq(0.75,dim(GrpGT2.Q)[1]-0.25,1.0),y=q75u,xend=seq(1.25,dim(GrpGT2.Q)[1]+0.25,1.0),yend=q75u),inherit.aes = FALSE,colour = "red",size = 1.5)+
#   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+ ylim(NA, 50)
```
...
salP1<- ggplot(dat.u,aes(x=Salinity,y=ValueStd))+
  ...
  scale_colour_discrete(limits = c("BE","LT","SE"),name="Country")+#limits = c("BE","LT","SE"),
  theme_bw()+
  theme(legend.position = "bottom")+
  ylab("TP (µg/L) GM boundary")
  #ylim(NA, 70) 
salP1
...
## A7.2 Appendix Total phosphorus (transitional waters)



```{r fig.width = 8.23, fig.height =7, fig.cap=CapA1}
bk <- c(10,50,200)
ymin <-10
ymax <- 200
...
# add lower point of range as blue symbol  
    ...
    ylab(ylb)+xlab("")+
    facet_wrap(~ ICcode.TRAC,ncol=3,nrow=6, scales="free_x") +
    theme_bw() +
